...
// Transactions are entity types 37, 45, 46, 47, linked via ZACCOUNT2, using ZAMOUNT1
func (db *DB) GetTransactions(accountID int64, limit int) ([]Transaction, error) {
	...
	if accountID > 0 {
		query = `
			SELECT Z_PK, ZAMOUNT1, ZDATE1, ZDESC2, ZACCOUNT2
			FROM ZSYNCOBJECT
			WHERE Z_ENT IN (37, 45, 46, 47) AND ZAMOUNT1 IS NOT NULL AND ZACCOUNT2 = ?
		...
		query = `
			SELECT Z_PK, ZAMOUNT1, ZDATE1, ZDESC2, ZACCOUNT2
			FROM ZSYNCOBJECT
			WHERE Z_ENT IN (37, 45, 46, 47) AND ZAMOUNT1 IS NOT NULL
			ORDER BY ZDATE1 DESC
			LIMIT ?
		`
